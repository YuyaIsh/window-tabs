...
function App() {
  ...
  return <main className={isController ? "controller-shell" : "group-host"}>
    <section className={nativeDragId ? "tabbar native-drag" : "tabbar"} aria-label="window-tabs" onMouseDown={startHostDrag} onDragOver={(event) => event.preventDefault()} onDrop={dropTabOnGroup}>
      ...
      <button className="add" aria-label="ウィンドウを追加" aria-keyshortcuts="F8 Ctrl+Shift+A" title="ウィンドウを追加 (F8)" onClick={() => { if (!isController) sendCommand({ type: "open-picker", groupId: group?.id }); else { setPickerContext(closedPickerContext()); void refresh(); setPicker(true); } }}>＋</button>
